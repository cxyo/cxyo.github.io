...
如果需要显示ETF实时涨幅数据和智能交易策略，需要启动Python定时爬虫服务。
...
- **非交易时间**：仅在启动时执行一次
- **输出文件**：jsl.json（ETF实时涨幅数据）
- **前端自动加载**：页面会自动读取jsl.json文件显示最新数据

### 策略计算逻辑

- **买入**：今年涨幅>0 且 上一日涨跌<0 且 上两日涨跌<0 且 实时涨幅>0
- **卖出**：上一日涨跌>0 且 上两日涨跌>0 且 实时涨幅<0
- **持有**：其他情况
